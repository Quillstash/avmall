/**
 * Mock data for storefront + DB seed.
 *
 * The product catalogue here is curated from Avmall's real Bumpa export
 * (~120 SKUs) so the storefront, admin, and DB seed all share one canonical
 * dataset. Imagery is the legacy Bumpa CloudFront CDN — Phase 5 migrates
 * everything to Cloudflare R2.
 *
 * Money is integer kobo throughout. The compact `mp(...)` helper expands
 * each row into a fully-typed Product with a default variant + theme.
 */
...
export type Product = {
  ...
  /** Primary product image (Bumpa CloudFront CDN). */
  imageUrl: string;
...
/** Look up a product by slug. Used by admin product editor. */
